...
# föll -------------------------------------------------------------------------
pretty_coords <- function(x, nw = "N") {
  paste0(str_sub(x,1,2),"°",str_sub(x,3,4),"'",str_sub(x,5)," ", nw)
}
```
...
```{r}
fs::dir_tree("/net/hafgola/var/ftp/pub/data/gpkg")
```
...
```{r}
GP <- 
  read_csv("external/log/grunnpunktar.csv",
           col_types = c("innc"))
GP %>% 
  mutate(breidd = pretty_coords(breidd),
         lengd = pretty_coords(lengd, "V")) %>% 
  knitr::kable()
```
...
GP.tb.sker <- 
  tribble(~nafn, ~lengd, ~breidd,
          'Kolbeinsey', 184058.70, 670859.57,
          'Hvalbakur',  131637.71, 643545.42) %>% 
  mutate(region = tolower(nafn))
GP.tb.grimsey <- 
  read_csv("external/grimsey.cvs")
GP.tb.grimsey <-
  GP.tb.grimsey %>% 
  bind_rows(GP.tb.grimsey,
            tibble(breidd = GP.tb.grimsey$breidd[1],
                   lengd = GP.tb.grimsey$lengd[1])) %>% 
  mutate(nafn = "Grímsey",
         region = "grimsey")
GP.pt <- 
  bind_rows(GP.tb, GP.tb.sker, GP.tb.grimsey) %>% 
  mutate(ref = "Lög 1979_0041",
         link = "https://www.althingi.is/lagas/nuna/1979041.html")
if(SAVE) {
  GP.pt %>% write_csv("/net/hafgola/var/ftp/pub/data/csv/grunnpunktar.csv")
}
# read from scatch
GP.pt <-read_csv("ftp://ftp.hafro.is/pub/data/csv/grunnpunktar.csv")
...
if(SAVE) {
  GP.pt %>%
    st_write(paste0(gpkg.path, "grunnpunktar.gpkg"), append = FALSE)
}
GP.po <- 
  bind_rows(GP.pt %>% 
              # not sker because they are just points
              filter(region %in% c("meginland", "grimsey")) %>% 
              group_by(region) %>% 
              summarise(do_union = FALSE) %>% 
              st_cast("LINESTRING") %>% 
              st_cast("POLYGON"),
            GP.pt %>% 
              filter(region %in% c("kolbeinsey", "hvalbakur")) %>% 
...
if(SAVE) {
 GP.po %>%
   st_cast("LINESTRING") %>%
   st_write(paste0(gpkg.path, "grunnlina.gpkg"), append = FALSE)
}
...
if(SAVE) {
 landhelgi %>%
    st_write(paste0(gpkg.path, "landhelgi.gpkg"), append = FALSE)
}
```
...
# eez midlinur
midlinur <- 
  read_csv("ftp://ftp.hafro.is/pub/data/csv/midlinur_eez.csv",
           col_types = c("cnncc")) %>%
  mutate(lon = -GC(lengd),
         lat =  GC(breidd))
...
```{r}
VP <- 
  read_csv("external/vidmidunarpunktar.csv",
           col_types = c("innci"))
if(SAVE) {
